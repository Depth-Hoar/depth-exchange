...
contract('Exchange', ([deployer, feeAccount, user1, user2]) => {
    ...
    describe('depositing Ether', async () => {
        ...
        it('emits a Deposit event', async () => { // can add this to helpers.js it's an event
                const log = result.logs[0]
                log.event.should.eq('Deposit')
                const event = log.args
                event.token.should.equal(ETHER_ADDRESS, 'token adress is correct')
    ...
    describe('widrawing Ether', async () => {
        let result
        let amount
        ...
        describe('success', async () => {
            ...
            it('emits a "Withdraw" event', async () => { // can add this to helpers.js it's an event
                const log = result.logs[0]
    ...
    describe('depositing tokens', () => {
        ...
        describe('success', () => {
            ...
            it('emits a Deposit event', async () => { // can add this to helpers.js it's an event
                ...
                log.event.should.eq('Deposit')
                const event = log.args
                event.token.should.equal(token.address, 'token adress is correct')
                event.user.should.equal(user1, 'user address is correct')
                event.amount.toString().should.equal(amount.toString(), 'amount is correct')
                event.balance.toString().should.equal(amount.toString(), 'balance is correct')
            })
        ...
        describe('failure', () => {
            it('rejects Ether deposits', async () => {
                await exchange.depositToken(ETHER_ADDRESS, tokens(10), { from: user1 }).should.be.rejectedWith(EVM_REVERT) // don't allow Ether deposits 
            })
